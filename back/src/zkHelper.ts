...
export const CardClassField: Record<string, bigint> = {
    knight: 1n,
    herald: 2n,
    baron: 3n,
    bishop: 4n,
...
export const generateSalt = (): bigint => {
    const bytes = randomBytes(31); // 31 bytes = safe within bn254 field
    return BigInt('0x' + bytes.toString('hex'));
};

/**
 * Represents a card commitment that is safe to share publicly.
 */
export interface CardCommitment {
    cardClass: string;
    salt: bigint;
    /** Pedersen hash as an unprefixed hex string, ready for the circuit. */
    commitment: string;
}
 ...
 */
export const commitCard = async (cardClass: string): Promise<CardCommitment> => {
    const classField = CardClassField[cardClass.toLowerCase()];
 ...
 */
export const encodePublicInputs = (commitment: bigint, declaredGuess: bigint): Buffer => {
